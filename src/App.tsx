...
const RESUME_URL = "https://drive.google.com/file/d/1buHDC3ZoaXvupAcFtA5GaWOPieP6rEYK/view";
      ...
      {open && (
        <div className="md:hidden border-t border-hairline bg-background/95 backdrop-blur">
          <nav className="mx-auto max-w-6xl px-6 py-4 flex flex-col gap-1">
            {navLinks.map((l) => (
              <a key={l.href} href={l.href} onClick={() => setOpen(false)} className="py-3 text-base text-foreground/90 hover:text-accent-blue transition-colors">
                {l.label}
              </a>
            ))}
      ...
      <div className="mx-auto max-w-6xl px-6">
        <div className="max-w-3xl">
          ...
          <h1 className="mt-8 text-4xl sm:text-6xl lg:text-7xl font-bold tracking-tight leading-[1.05]">
            Short-Form<br />Creative <span className="text-accent-blue">Producer.</span>
          </h1>
          <p className="mt-6 text-base sm:text-lg text-muted-foreground max-w-xl leading-relaxed">
            3 years of building audiences, breaking down complex topics, and leading end-to-end production.
          </p>
          <div className="mt-10 grid grid-cols-3 gap-4 max-w-xl">
            {[{ num: "1.6M+", label: "Followers Grown" }, { num: "1.5M+", label: "Avg. Views Per Reel" }, { num: "750K+", label: "YouTube Subscribers Built" }].map((s) => (
              <div key={s.label}>
...
function WorkSamples() {
  const samples = [
    { icon: FileText, title: "Sample Script", desc: "A short-form script breaking down a complex topic.", href: "https://www.notion.so/Yash-s-Portfolio-1bfc2ca2cb924936b1393f2732218f3e?source=copy_link" },
    { icon: ClipboardList, title: "SOP Document", desc: "Operating playbook for running a content page.", href: "https://canva.link/5qb8v0x48dfkl2x" },
  ];
  return (
    <section className="py-16">
      <div className="mx-auto max-w-6xl px-6">
        ...
        <div className="mt-8 grid sm:grid-cols-2 gap-5">
          {samples.map((s) => (
            <a key={s.title} href={s.href} target="_blank" rel="noreferrer" className="block rounded-2xl border border-hairline p-6 bg-white/[0.02] hover:border-accent-blue/50 transition-colors">
              <s.icon className="text-accent-blue" size={22} />
              <div className="mt-4 text-lg font-medium">{s.title}</div>
              <p className="mt-2 text-sm text-muted-foreground">{s.desc}</p>
              <span className="mt-5 inline-flex items-center gap-2 text-sm text-accent-blue hover:opacity-80">View / Download →</span>
            </a>
          ))}
        </div>
      </div>
      ...
      <div className="mx-auto max-w-6xl px-6">
        ...
        <div className="mt-12 grid md:grid-cols-2 gap-6">
          {testimonials.map((t, i) => (
            <a key={i} href={t.linkedin} target="_blank" rel="noreferrer" aria-label={`${t.name} on LinkedIn`}
              ...
              <div className="flex items-center gap-4">
                {t.image ? (
                  <img src={t.image} alt={t.name} className="h-12 w-12 sm:h-[60px] sm:w-[60px] rounded-full object-cover ring-1 ring-white/10" />
                ) : (
  ...
  return (
    <section id="contact" className="py-20 sm:py-28">
      <div className="mx-auto max-w-4xl px-6 text-center">
        ...
        <p className="mt-5 text-muted-foreground max-w-xl mx-auto">Open to collaborations, freelance projects, and full-time opportunities.</p>
        <div className="mt-10 flex flex-wrap justify-center gap-3">
          <a href={MAILTO} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 bg-accent-blue text-white px-6 py-3 rounded-full text-sm font-medium hover:opacity-90 transition-opacity"><Mail size={16} /> Email Me</a>
          <a href={LINKEDIN_URL} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 border border-hairline px-6 py-3 rounded-full text-sm font-medium hover:bg-white/5 transition-colors"><Linkedin size={16} /> LinkedIn</a>
          <a href={CALENDLY_URL} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 border border-hairline px-6 py-3 rounded-full text-sm font-medium hover:bg-white/5 transition-colors"><Calendar size={16} /> Schedule a Call</a>
        </div>
      </div>
    </section>
  );
